...
species_list_df = pd.read_csv(data_path + "vog.species.list",
                              sep='\t',
                              header=0,
                              names=['SpeciesName', 'ID', 'Phage', 'Source', 'Version']) \
...
with engine.connect() as con:
    con.execute('ALTER TABLE `Species_profile` ADD PRIMARY KEY (`ID`);')
    con.execute('ALTER TABLE Species_profile  MODIFY  SpeciesName char(100) NOT NULL; ')
    con.execute('ALTER TABLE Species_profile  MODIFY  ID int(255) NOT NULL; ')
