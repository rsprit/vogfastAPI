...
#protein_list_df = protein_list_df.loc[np.random.choice(protein_list_df.index, 2000, replace=False)]
...
# separating proteins into 10 000 chunks
species_names_merged = []
chunk_size = 10000

print("Mining species names from Entrez")
for num in range(0, protein_list_df.shape[0], chunk_size):

    tax_ids_subset = tax_ids[num :num + chunk_size]
    handle = Entrez.efetch('taxonomy', id=tax_ids_subset, rettype='xml')
    response = Entrez.read(handle)

    species_names = [entry.get('ScientificName') for entry in response]
    species_names_merged = species_names_merged + species_names


#add species to the protein table
protein_list_df.insert(3, "Species_name", species_names_merged)
